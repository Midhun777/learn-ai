...
const MentorDashboard = () => {
    ...
    const fetchUsers = async () => {
        try {
            setLoading(true);
            const res = await axios.get('http://localhost:5000/api/mentor/users');
            setUsers(res.data);
        ...
        <div className="p-6 lg:p-10 max-w-[1600px] mx-auto space-y-8 animate-fade-in">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                ...
                <div className="flex items-center gap-3">
                    <div className="bg-white p-2 rounded-xl shadow-soft border border-slate-100 flex items-center gap-3 px-4">
                        <Users className="w-4 h-4 text-slate-400" />
                        <span className="text-xs font-bold text-slate-600">{users.length} Students</span>
                    </div>
